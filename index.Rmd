...
plot1 <- ggplot()+
  geom_polygon(data = state_geom, aes(x = long, y = lat, group = group), col = "#ba5454")+
  coord_fixed(1.5)+
...
```
9. Create an interactive plot of solar projects using leaflet
```{r, fig.width=8, fig.height=3, echo = FALSE, warning = FALSE}
library(leaflet)
leaflet <- leaflet(NREL_Data)%>%
  addTiles()%>%
  addAwesomeMarkers(lng = ~Longitude, lat = ~Latitude, popup = ~as.character(Project), clusterOptions = markerClusterOptions())
```
...
plot3
```
An interactive plot of solar projects using leaflet
```{r, fig.width=8, fig.height=3, echo = FALSE, warning = FALSE}
leaflet
```
