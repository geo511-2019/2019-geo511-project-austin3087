...
5. Create two maps: one with the counties colorized red or blue to show political affiliation, and the other displaying a color gradient (fill in ggplot) to display the public perception for each county.    
...
kwh_mean_state <- DSIRE_program_table%>%
  group_by(name.y)%>%
  filter(units == "$")%>% #note only using $ values that are given, multiple other units given in table but needed to be filtered for this analysis
...
#Plot just Community Locations add state geometry to DSIRE data to plot
plot1 <- ggplot()+  
